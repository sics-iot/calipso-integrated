...
import it.unipr.iot.calipso.http.client.dto.WorldSensingServerData;
...
public class WorldsensingServerRequest {
	...
	public static ResponseEntity<String> postData(String baseURL, WorldSensingServerData data) {
		try{
			...
			long signal = 100;
			String station = "54C";
			URI targetUrl = UriComponentsBuilder.fromUriString(baseURL).queryParam("id", id).queryParam("time", seconds).queryParam("duplicate", duplicate).queryParam("signal", signal).queryParam("station", station).queryParam("data", data.getData()).build().toUri();
